...
class MSnake:
    ...
    def _Prepare(self):
        ...
        for i in range(self.BodyLength):
            self.Body.append([self.BodyLength - i - 1, 0])
        # 设置为默认方向
    ...
    def CanMove(self, NextStepLoc):
        return (
            (NextStepLoc[0] >= 0)
            and (NextStepLoc[0] < self.AGround.GetGroundWidth())
            and (NextStepLoc[1] >= 0)
            and (NextStepLoc[1] < self.AGround.GetGroundHeight())
            and (not self.NodeInSnake(NextStepLoc))
        )
    ...
    def SetDirect(self, NewDirect):
        ...
        if self.Direc != NewDirect:
            if (
                (self.Direc == MSDirec_Up)
                and (NewDirect != MSDirec_Down)
                or (self.Direc == MSDirec_Down)
                and (NewDirect != MSDirec_Up)
                or (self.Direc == MSDirec_Left)
                and (NewDirect != MSDirec_Right)
                or (self.Direc == MSDirec_Right)
                and (NewDirect != MSDirec_Left)
            ):
